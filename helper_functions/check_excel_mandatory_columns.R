...
check_excel_mandatory_columns <- function(folder_path, cols) {
  ...
  if (!dir.exists(folder_path)) {
    return(data.table(Folder = basename(folder_path), Result = "Invalid", Comment = "Concept folder does not exist in the main app directory."))
  }
  ...
  if (length(folder_parts) < 3) {
    return(data.table(Folder = folder_name, Result = "Invalid", Comment = "Concept folder name does not have at least three elements."))
  }
  ...
  if (length(excel_files) == 0) {
    return(data.table(Folder = folder_name, Result = "Invalid", Comment = "No Concept Excel file found in the folder."))
  }
  ...
  if (length(matching_files) == 0) {
    return(data.table(Folder = folder_name, Result = "Invalid", Comment = "No matching Concept Excel file found."))
  }
  ...
  if (is.null(sheet_names)) {
    return(data.table(Folder = folder_name, Result = "Invalid", Comment = "Error reading Concept Excel file."))
  }
  
  # Check if the sheet with the same name as the Excel file exists
  if (!(expected_filename %in% sheet_names)) {
    return(data.table(Folder = folder_name, Result = "Invalid", Comment = "Sheet with expected name not found in the Concept Excel file."))
  }
  ...
  if (length(missing_columns) > 0) {
    return(data.table(Folder = folder_name, Result = "Invalid", 
                      Comment = paste("Missing mandatory columns:", paste(missing_columns, collapse = ", "), ". Review requirements and add the necessary column names.")))
  }
  
  # If all checks pass
  return(data.table(Folder = folder_name, Result = "Valid", Comment = "All Excel files in concepts contain the required columns."))
}
